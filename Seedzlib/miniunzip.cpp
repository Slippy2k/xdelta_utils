...
bool zip::ZipArchiveInput::Open( tstring const& archiveName, bool caseSensitive )
{
...
bool zip::ZipArchiveInput::LocateAndReadFile( tstring const& fileName, BYTE*& pMemoryBlock, size_t& size )
{
	int err = UNZ_OK;
#ifdef _UNICODE
...
bool zip::ZipArchiveInput::ReadFile( tstring const& fileName, BYTE*& pMemoryBlock, size_t& size )
{
	tstring fileNameKey = fileName;
	if( !m_caseSensitive )
		for (size_t i = 0; i < fileNameKey.size(); ++i )
			fileNameKey[i] = towlower(fileNameKey[i]);
...
bool zip::ZipArchiveInput::ReadFile(tstring const& fileName, FileMemory & rMemory)
{
	tstring fileNameKey = fileName;
	if (!m_caseSensitive)
		for (size_t i = 0; i < fileNameKey.size(); ++i)
	...
	return ReadCurrentFile(fileName, rMemory);
}

bool zip::ZipArchiveInput::ExtractAll(const tstring strTargetpath, std::function<bool(const tstring, FileMemory &)> CallBack)
{

	for each(auto zipEntry in m_nameToEntry)
	{
		unz_file_pos pos;
		pos.pos_in_zip_directory = zipEntry.second.pos_in_zip_directory;
		pos.num_of_file = zipEntry.second.num_of_file;

		int err = unzGoToFilePos(uf, &pos);
		if (err != UNZ_OK)
		{
			m_errorMessage << "Can't go to file " << zipEntry. first << std::endl;
			return false;
		}
		FileMemory WriteFile;
		if (false == ReadCurrentFile(zipEntry.first, WriteFile))
		{
			return false;
		}
		tstring strFilePath = strTargetpath + _T("\\") + zipEntry.second.strFileName;

		
		if (false == CallBack(zipEntry.second.strFileName, WriteFile))
		{
			return false;
		}
	}
...
bool zip::ZipArchiveInput::ReadCurrentFile(tstring const& fileName, FileMemory & rMemory)
{
	int err = UNZ_OK;
	...
	if (m_strPasswd.size() > 0) password = m_strPasswd.c_str();
	...
	rMemory.Set( (BYTE*)buf );
	rMemory.SetSize(size_buf);
	return true;
}

bool zip::ZipArchiveInput::ReadCurrentFile( tstring const& fileName, BYTE*& pMemoryBlock, size_t& size )
{
	int err = UNZ_OK;
...
bool zip::ZipArchiveInput::Index()
{
	...
	while (err == UNZ_OK)
	{
		...
		if(err == UNZ_OK)
		{
			ZipEntry zipEntry;
#ifdef _UNICODE
			tstring fileNameKey = utf_convert::as_wide( szCurrentFileName );
#else
			tstring fileNameKey = szCurrentFileName;
#endif
			zipEntry.strFileName = fileNameKey;
			if( !m_caseSensitive )
				for (size_t i = 0; i < fileNameKey.size(); ++i )
...
tstring zip::ZipArchiveInput::ErrorMessage() const
{
