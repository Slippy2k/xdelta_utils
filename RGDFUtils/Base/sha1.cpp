...
bool CharToByte( TCHAR c, unsigned char& byte )
{
	if( c >= _T('0') && c <= _T('9') )
	{
		byte = c - _T('0');
...
#ifdef _UNICODE
#define STR_LEN(x) wcslen(x)
#else
#define STR_LEN(x) strlen(x)
#endif


bool StringToSHA1( const TCHAR* parse, Sha1& sha1 )
{
	size_t size = STR_LEN(parse);
